        ...
        .service('CommonUtils', ['Debounce', '$window', '$q', '$parse', '$cookies', function (Debounce, $window, $q, $parse, $cookies) {
            ...
            CommonUtils.prototype = {
                ...
                loginContext: function () {
                    var id, username, employeeName, orgId, orgName;
                    if ($cookies['eccrmContext.id']) {
                        id = $cookies['eccrmContext.id'];
                        orgId = $cookies['eccrmContext.orgId'];
                        orgName = decodeURI(decodeURI($cookies['eccrmContext.orgName']));
                ...
                loading: function (promise, title, successCallback, scope, failCallback) {
                    var elm = $('<div style="position: fixed;top:0;left: 0;width: 100%;height: 100%;opacity: 0;z-index: 99998;">' +
                        '<div style="position:absolute;height: 100%;width: 100%;background-color: #000;opacity: 0.3;"></div>' +
                 ...
                 * @param idStr ID名称
                 * @param pidStr 上级ID名称
                 * @param childrenStr 子集合的名称
                 * @returns {Array} 新的树形的数组
                 */
                array2Tree: function (array, idStr, pidStr, childrenStr) {
                    var r = [], hash = {}, pid = pidStr, children = childrenStr, i = 0, j = 0, len = array.length;
                    for (; i < len; i++) {
            ...
            return {
                ...
                link: function (scope, element, attr, ctrl) {
                    var defaults = {
                        ...
                        pageSize: [5, 10, 15, 20, 30, 40, 50, 100, 200, 500, 1000],
                        start: 0,
                        limit: 15,//每页显示的数量
                        total: 0,//总数据
                        opacity: 1,//透明度
                    ...
                    promise.then(function (pagerOptions) {
                        ...
                        var destroy = scope.$watch('pager.currentPage', function (value, oldValue) {
                            ...
                            if (value == 0 || value > pager.totalPage) {
...
(function ($) {
    $(function () {
        // 获得查询按钮
        var btn = $('.list-condition .header-button .btn:contains("查询")');
