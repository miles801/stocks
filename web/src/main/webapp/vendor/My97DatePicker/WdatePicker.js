...
// 一下是基于My97Datepicker封装的angularjs指令
// 该指令作用域'eccrm.angular'模块，所以需要先引入对应的js，并将该js放到其后
//
(function () {
    if (!angular) {
        return;
    ...
    if (app == null) {
        alert('没有获得angularjs的模块[eccrm.angular]，请确保该js在[eccrm.angular]模块js之后!');
        return;
    }
    // 基于my97封装的angularjs插件，基本功能是在显示时间选择器时，自动定位到当前的元素下
    // 使用方式<input eccrm-my97="options">
    // 配置方式：options为一个对象
    //  dateFmt:'',要显示的格式，默认值为'yyyy-MM-dd'
    //  其他配置项：参考my97的官方文档
    app.directive('eccrmMy97', ['CommonUtils', '$filter', function (CommonUtils, $filter) {
        return {
            require: '?ngModel',
            scope: {
                options: '=eccrmMy97'
            ...
            link: function (scope, element, attr, ctrl) {
                if (!angular.isFunction(WdatePicker)) {
                    alert('没有检测到My97Datepicker插件!');
                    return false;
                }
                var id = element.attr('id');
                if (id == undefined) {
                    id = CommonUtils.randomID(6);
                    element.attr('id', id);
                }
                // 获取当前元素的坐标
                ...
                // 延迟获取配置对象
                var promise = CommonUtils.parseToPromise(options);
                promise.then(function (conf) {

                    // 格式化回显数据
                    ctrl.$formatters.unshift(function (modelValue) {
                        if (modelValue) {
                            var format = conf.dateFmt || 'yyyy-MM-dd';
